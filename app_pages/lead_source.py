"""Lead source module — calls its own backend over HTTP (LEAD_SOURCE_BACKEND_URL,
falling back to the legacy BACKEND_URL, then the bundled config's default_url).
Ported unchanged from the standalone leadscraping-frontend: a sidebar campaign
form (geography, industries, roles, run controls) -> runs V1/V2 sourcing on the
backend -> shows sources, qualified leads, CSV export and the database handoff.

Non-secret UI configuration (states, industries, roles, slider limits...) comes
from lead_source/streamlit_config.json, optionally overridden from S3
(STREAMLIT_CONFIG_S3_URI) or a local file (STREAMLIT_CONFIG_FILE).

Deliberately does NOT use utils.api.resolve_backend_url: that helper falls back
to BACKEND_BASE_URL, which is the Ad generator's backend, not this one."""
import csv
import io
import json
import os
...
from lead_source.config_loader import load_config
...
BACKEND_URL = (os.getenv("LEAD_SOURCE_BACKEND_URL") or os.getenv("BACKEND_URL") or BACKEND_CONFIG["default_url"]).rstrip("/")

# st.set_page_config lives in the shell (streamlit_app.py); pages must not call it.
...
with center_cols[1]:
    st.subheader("Sourcing campaign")
    ...
    run_campaign = st.button("Run lead sourcing", type="primary", use_container_width=True, disabled=not industries or period_end < period_start)
